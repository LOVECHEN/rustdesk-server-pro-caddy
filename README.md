...
下面列出**全部支持的环境变量**——默认**整段都注释掉**（直接跑就行：无域名→网卡有公网IP 就试公网IP 证书、否则内网自签，`:21120` 直接可用，其余全用默认值）。要用哪项就取消注释哪项（给默认值没意义、还可能盖掉控制台里的配置）：
...
services:
  rustdesk-server:
    ...
    # 全部可选：默认整段注释掉——直接跑就是「无域名→网卡有公网IP 就试公网IP 证书、否则内网自签，:21120 直接可用」+ 各设置用默认值。
